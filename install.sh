...
# ---- 9. persist install env ------------------------------------------------
# Drop a small env file so any future re-run of helper scripts (or manual
# diagnostics) can recover the queue/printer/scan-share names without
# parsing CUPS state. Source with: `set -a; . /etc/airprint-v2/install.env; set +a`.
...
printf '\nTo re-run individual steps, see /opt/airprint-v2/scripts/.\n'
